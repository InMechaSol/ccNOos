...
void LinkAxisSPDStructArray(struct axisStruct* mystruct, struct SPDStruct* myArray)
{
    for (int i = mcsNone; i < mcsEND; i++)
    {
        ...
        case mcsActMotionState: myArray[i] = {i,    "Actual Motion State",  "(-1,0,1 neg-stop-pos)",                 1,SIGNED_TYPE,  sizeof(mystruct->Planning.actualMotionState),   &mystruct->Planning.actualMotionState}; break;
        case mcsDesiredMotionState: myArray[i] = {i,"Desired Motion State", "(-1,0,1 neg-stop-pos)",                 1,SIGNED_TYPE,  sizeof(mystruct->Planning.desiredMotionState),  &mystruct->Planning.desiredMotionState};break;
        ...
        case mcsCMDgenDesiredMode: myArray[i] = { i,"Gen. Mode Desired",    "(0-3 none-pulse-train-sine)",                 0,UNSIGNED_TYPE,   sizeof(mystruct->Planning.cmdGenerator.desiredGenMode), &mystruct->Planning.cmdGenerator.desiredGenMode };      break;
        case mcsCMDgenOffset: myArray[i] = { i,     "Gen. Offset",          "(0-3 none-pulse-train-sine)",                 0,FLOAT_TYPE,   sizeof(mystruct->Planning.cmdGenerator.offset), &mystruct->Planning.cmdGenerator.offset };      break;
